...
#+BEGIN_SRC emacs-lisp
  ...
  (setq init/theme-day   'leuven
        init/theme-night 'zenburn)

  (load-theme init/theme-day)
  (disable-theme init/theme-day)

  (load-theme init/theme-night)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (defun toggle-theme ()
    (interactive)
    (let* ((is-day (eq (car custom-enabled-themes) init/theme-day))
           (this-theme  (if is-day init/theme-day   init/theme-night))
           (other-theme (if is-day init/theme-night init/theme-day)))

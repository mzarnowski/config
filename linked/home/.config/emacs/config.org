...
(cl-defun +org-refile-completing-read (workspace)
  (let ((targets (+org-refile-targets workspace)))
    ...
    (let ((table (+hash-map targets
			    :key #'+org-refile-completion-item
			    :test #'equal)))
...
(cl-defun +org-refile-completion-item (target)
  (let ((group      (plist-get target :group))
...
(cl-defun +org-refile-target-marker (buffer target)
  (with-current-buffer buffer
    (save-excursion
      (goto-char (point-min))
      (+org-refile-find-or-create target))))

(cl-defun +org-refile-find-or-create (target)
  (cl-loop with outline = (plist-get target :outline)

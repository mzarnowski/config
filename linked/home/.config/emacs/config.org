...
(cl-defun +org-refile-completing-read (workspace)
  (let ((table (+hash-map +org-refile-targets
			  :key (lambda (it) (apply #'+completion-item it))
			  :test #'equal)))
    (car (+complete table :prompt "Target: "))))

(cl-defun +org-root-heading (buffer name)
  (with-current-buffer buffer
    (save-excursion
      (or (org-find-exact-headline-in-buffer name)
	  (progn
	    (goto-char (point-min))
	    (org-insert-heading)
	    (insert name)
...
(cl-defun +org-refile (workspace)
  ...
  (save-excursion
    ...
    (when-let* ((heading (+org-refile-completing-read workspace))
		(file    (+org-workspace-current-file workspace))
		(marker  (+org-root-heading (find-file-noselect file) heading)))
...
(define-derived-mode +org-capture-mode org-mode "Capture")

...
** +complete
#+begin_src emacs-lisp
(cl-defun +completion-item (label &key group)
  (let* ((group (or group "default"))
	 (text (concat group " " label)))
    (propertize text :group group :label label)))

(cl-defun +complete (table &key prompt)

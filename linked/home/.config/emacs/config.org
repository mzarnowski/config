...
(cl-defun kafka-topics-create (bootstrap-server name &optional (partitions 1) (replicas 1))
  (interactive (list kafka-bootstrap-server
		     (read-string "Topic Name: ")))
  (message (--> (kafka-topics-sh bootstrap-server "--create"
				 "--topic" name
...
(cl-defun kafka-topics-describe--parse-partitions ()
  (rx-let ((item (name value) (seq "\t" (literal name) ": " value))
	   ...
	   (leader   (item "Leader" (group-n 3 number)))   ;; broker id? same for replicas and isr
	   (replicas (item "Replicas" (group-n 4 number))) ;; TODO comma separated numbers
	   (isr  (item "Isr" (group-n 5 number))))         ;; TODO in-sync-replicas
...
*** kafka-consume
#+begin_src emacs-lisp
(cl-defun kafka-consume (bootstrap-server topic)
  (interactive
   (let* ((bootstrap-server (read-string "Bootstrap server: "))
	  (topics (-map #'kafka-topic-name (kafka-topics-describe bootstrap-server)))
	  (topic (completing-read "Topic: " topics)))
     (list bootstrap-server topic)))
  (with-current-buffer (generate-new-buffer (format "kafka:%s@%s" topic bootstrap-server))
    (read-only-mode)
...
(defun +org-drawer-start-regexp (name)
  (rx line-start (* (or " " "	")) ":" (literal name) ":" (*? whitespace) line-end))

...
** +complete
#+begin_src emacs-lisp
(cl-defun +completion-item (label &key group &allow-other-keys)
...
#+begin_src emacs-lisp
(cl-defun +org-quote-refile-targets (workspace)
  (cl-flet ((parse () (let ((outline (org-get-outline-path t))
			    (book    (book (org-entry-get nil "ISBN"))))
			(list (car (last outline))
			      :group  "projects - books"
			      :book   book
			      :marker (point-marker)))))
    (org-ql-select (+org-workspace-current-file workspace)
      `(property "ISBN") :action #'parse)))

(cl-defun +org-quotes (workspace)
  (save-excursion
    (goto-char (point-min))
    (let ((quotes '()))
      )))
...
(cl-defun +org-refile-target-marker (buffer name properties)
  (with-current-buffer buffer
    (save-excursion
	  ...
	  (progn
	    ...
	    (insert " " name)
            (-each properties (lambda (it) (apply #'org-set-property it)))
	    (org-back-to-heading)
	    (point-marker))))))
#+end_src
...
(cl-defun +org-refile-book-targets (workspace)
  (let ((books (book-catalogue))
	(targets '()))
    (maphash (lambda (isbn book) (push (list (book-title book) :group "projects - books") targets)) (book-catalogue))
    targets))
...
#+begin_src emacs-lisp
(cl-defun +org-capture-template (&key keys workspace)
  (doct `((,workspace
	  :keys ,keys
	  :type entry
	  :file ,(+org-workspace-current-file workspace)
...
(cl-defun +org-next-captured (workspace)
  (interactive '("mzarnowski"))
  (let ((files (+org-workspace-files workspace "inbox")))
    (if files
	(find-file (car files))
...
(use-package org
  ...
  (setq org-capture-templates (-list (+org-capture-template :keys "m" :workspace "mzarnowski")))

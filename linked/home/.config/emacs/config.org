...
#+BEGIN_SRC emacs-lisp
(cl-defun +completing-read (table &key prompt)
  (cl-labels ((group (candidate transform)
    ...
    (let ((key (completing-read prompt #'collection)))
      (gethash key table))))
...
(defcustom +org-refile-default-target-query
  '(or (heading "inbox") (heading "journal") (heading "insight") (heading "bookmarks")
       (parent (heading "milestones")))
  "TODO")
...
(cl-defun +org-workspace-files (workspace)
  (f-files (f-join +org-workspace-root workspace "org")
...
(org-link-set-parameters +org-link-schema
                         :follow #'+org-link-follow)

(cl-defun +org-completing-read (entries &key prompt (key #'+org-refile-completing-target))
  (let ((table (+hash-map entries :key key :test #'equal)))
    (+completing-read table :prompt prompt)))

(cl-defun +org-link-completing-read (workspace)
  (let* ((buffers-files (+org-workspace-files workspace))
	 (entries (org-ql-select buffers-files +org-link-query
		   :action (lambda () (list (point-marker)
					    (cons :outline (org-get-outline-path t))
					    (cons :id      (org-id-get)))))))
    (+org-completing-read entries :prompt "Link: ")))

(cl-defun +org-link (workspace)
  (interactive '("mzarnowski"))
  (unless (derived-mode-p 'org-mode))
  (if-let* ((selected (+org-link-completing-read workspace)))
      (let ((id (alist-get :id selected))
	    (name (car (last (alist-get :outline selected)))))
...
(cl-defun +org-refile-completing-target (it)
  (let* ((outline (alist-get :outline (cdr it)))
	 (name  (s-join "/" outline))
	 (group (cond
		 ((string= (nth 0 outline) "notes") "notes")
		 ((string= (nth 0 outline) "project") (nth 1 outline))
		 (t "general"))))
    (print (propertize name 'group group))))

(defcustom +org-workspace-current-file-name "current" "TODO")
(cl-defun +org-workspace-current-file (workspace)
  (f-join +org-workspace-root workspace "org" (concat +org-workspace-current-file-name ".org")))
(cl-defun +org-template (&key keys workspace)
  (doct `((,workspace
	  :keys ,keys
	  :type entry
	  ...
	  :template ("* %{todo-state} %?"
		     ...
		     ":CREATED-ON: %<%FT%R%z>"
		     ":END:")
	  :hook org-hide-drawer-all
	  :before-finalize save-buffer))))

(cl-defun +org-refile-available-targets (workspace &key (query +org-refile-default-target-query))
  (let ((buffers-files (list (+org-workspace-current-file workspace))))
    (org-ql-select buffers-files query
      :action (lambda () (list (point-marker)
			       (cons :outline (org-get-outline-path t)))))))

(cl-defun +org-refile-completing-read (workspace)
  (let ((table (+hash-map (+org-refile-available-targets workspace)
			  :key #'+org-refile-completing-target
			  :test #'equal)))
    (car (+completing-read table :prompt "Target: "))))

(cl-defun +org-refile (workspace)
  (interactive '("mzarnowski"))
  (when-let ((marker (+org-refile-completing-read workspace)))
    (while (not (org-id-get)) (outline-up-heading 1))
    (let ((name (org-with-point-at marker (nth 4 (org-heading-components))))
	  (file (buffer-file-name (marker-buffer marker))))
    (org-refile nil nil (list name file nil marker)))))

(cl-defun +org-capture (workspace)
  (interactive '("mzarnowski"))
  (let ((keys (when (= 1 (length org-capture-templates))
	       (caar org-capture-templates))))
    (org-capture nil keys)))

(cl-defun +org-capture-refile (workspace)
  (interactive '("mzarnowski"))
  (let ((org-capture-is-refiling t))
    (and (+org-refile workspace)
	 (kill-buffer))))
...
(use-package org
  ...
  (setq org-capture-templates (-list (+org-template :keys "m" :workspace "mzarnowski")))

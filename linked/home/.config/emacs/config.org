...
** +complete
#+begin_src emacs-lisp
(cl-defun +completion-item (&key path group (annotation nil) &allow-other-keys)
  (let* ((label (s-join " - " path))
...
(cl-defun +org-refile-target-marker (buffer target)
  (let ((path       (plist-get target :path))
    ...
    (with-current-buffer buffer
      (save-excursion
	(goto-char (point-min))
	(+org-refile-find-or-create path properties)))))

(cl-defun +org-refile-find-or-create (path properties)
  (cl-loop for name in path
	   for level = (if level (1+ level) (org-current-level))
	   for marker = (+org-search-heading level name)
	   while marker

	   finally do (unless marker
			(cl-loop for name in (-drop (1- level) path)
				 do (org-insert-subheading nil)
				 do (insert name))
...
(cl-defun +org-refile-book-target (book)
  (list :path (list (book-title book))

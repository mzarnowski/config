...
(cl-defun +org-workspace-current-file (workspace)
  (f-join +org-workspace-root workspace (concat +org-workspace-current-file-name ".org")))
...
(cl-defun +org-refile-available-targets (workspace &key (query +org-refile-default-target-query))
  (let ((buffers-files (list (f-join +org-workspace-root workspace "current.org"))))
    (org-ql-select buffers-files query
      :action (lambda () `(,(point-marker) (:outline . ,(org-get-outline-path t)))))))

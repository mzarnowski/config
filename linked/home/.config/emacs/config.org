...
(defcustom +org-refile-default-target-query
  '(or (heading "inbox") (heading "journal")
       (parent (heading "topic")))
  "TODO")
...
(cl-defun +completing-read (table &key prompt (group nil))
  (cl-labels ((group (candidate transform)
		     (pcase transform
		       (`nil (when group (funcall group (gethash candidate table))))
		       (_    candidate)))
...
(cl-defun +org-refile-completing-read (workspace)
  (let ((table (+hash-map (+org-refile-available-targets workspace)
			  :key (lambda (it) (s-join "/" (alist-get :outline (cdr it))))
			  :value #'car
			  :test #'equal)))
    (+completing-read table
		      :prompt "Target: "
		      :group (lambda (it) (-> it marker-buffer buffer-name)))))

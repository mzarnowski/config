...
(cl-defun +org-link (workspace)
  ...
  (if-let* ((buffers-files (f-files (f-join +org-workspace-root workspace "org")
				 (lambda (it) (f-ext-p it "org"))))
	 ...
	 (selected nil))
    (atomic-change-group
      (if-let ((foo-id (org-id-get))
	       (marker (car selected))
	       (link (org-link-make-string (concat +org-link-schema ":" (alist-get :id selected))
					   (s-join "/" (alist-get :outline selected))))
	       (progn (insert link)
		      (with-current-buffer (marker-buffer marker)
			(save-excursion
			  (goto-char (marker-position marker))
			  (if (+org-goto-drawer +org-backlink-drawer-name)
			      (progn)
			    (progn
			      ;; move past properties or stay at the beginning of entry
			      (re-search-forward org-property-drawer-re)
			      ;; insert new drawer
			      (org-insert-drawer nil +org-backlink-drawer-name)
			    ))))))
    ))

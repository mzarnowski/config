...
(cl-defun +complete (table &key prompt (category nil))
...
(cl-defun +org-refile (workspace)
  ...
  (save-excursion
    ...
    (when-let* ((file     (+org-workspace-current-file workspace))
		(selected (+org-refile-completing-read workspace))
		(heading    (car selected))
		(properties (plist-get (cdr selected) :properties))
		(marker   (+org-refile-target-marker (find-file-noselect file) heading properties)))
  ...
  (with-current-buffer buffer
    (save-excursion
	  ...
	  (progn
	    ...
	    (insert " " name)
            (-each properties (lambda (it) (org-set-property (car it) (cdr it))))
	    (org-back-to-heading)
	    (point-marker))))))
#+end_src
...
(cl-defun +org-refile-book-target (book)
  (list (book-title book)
	...
	:properties (list (cons "ISBN" (book-isbn book)))))
